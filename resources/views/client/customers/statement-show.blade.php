...
@section('content')
@php
    $whatsappMessage = 'Hola ' . $customer->full_name . ', te compartimos tu estado de cuenta del periodo '
        . $from->format('d/m/Y') . ' - ' . $to->format('d/m/Y') . '. PDF: '
        . route('client.customers.statements.pdf', [$customer, $statement]);
@endphp

<div class="-mt-4 mx-auto max-w-7xl space-y-6">
    <div class="rounded-[24px] border border-slate-200 bg-white p-6 shadow-sm">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <p class="text-[10px] font-black uppercase tracking-widest text-slate-400">Estado de cuenta</p>
                <h1 class="mt-1 text-2xl font-black theme-text-heading">CLIENTE: {{ $customer->full_name }}</h1>
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <a href="https://wa.me/?text={{ rawurlencode($whatsappMessage) }}"
                   target="_blank"
                   rel="noopener"
                   class="rounded-xl bg-emerald-50 px-4 py-2.5 text-[11px] font-black uppercase tracking-widest text-emerald-700 transition hover:bg-emerald-100">
                    Enviar WhatsApp
                </a>
    ...
    <div class="rounded-[24px] border border-slate-200 bg-white p-6 shadow-sm">
        <div class="mb-5 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div class="grid grid-cols-1 gap-3 text-left sm:grid-cols-3 lg:min-w-[720px] lg:text-right">
                <div>
                    <p class="text-[9px] font-black uppercase tracking-widest text-slate-400">Saldo anterior</p>
                    <p class="mt-1 text-base font-black theme-text-heading">${{ number_format((float) $previousBalance, 2) }}</p>
                </div>
                <div>
                    <p class="text-[9px] font-black uppercase tracking-widest text-slate-400">Cargos del periodo</p>
                    <p class="mt-1 text-base font-black theme-text-heading">${{ number_format((float) $totalInvoiced, 2) }}</p>
                </div>
                <div>
                    <p class="text-[9px] font-black uppercase tracking-widest text-emerald-700">Abonos del periodo</p>
                    <p class="mt-1 text-base font-black text-emerald-700">${{ number_format((float) $totalPaid, 2) }}</p>
                </div>
                {{-- <div>
                    <p class="text-[9px] font-black uppercase tracking-widest text-rose-700">Balance final</p>
                    <p class="mt-1 text-base font-black text-rose-700">${{ number_format((float) $totalDebt, 2) }}</p>
                </div> --}}
            </div>
        </div>
