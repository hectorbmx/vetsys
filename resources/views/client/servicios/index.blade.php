                ...
                <thead>
                    <tr class="border-b border-slate-100 theme-surface-dark " >
                        <th class="px-6 py-3 text-[10px] font-black text-white uppercase tracking-widest">
                            <form method="GET" action="{{ route('client.servicios.index') }}" class="relative w-full max-w-xs">
                                <input type="hidden" name="per_page" value="{{ $perPage }}">
                                <span class="absolute inset-y-0 left-0 flex items-center pl-3 text-slate-400 text-[10px]">🔍</span>
                                <input type="text" name="q" value="{{ $search }}" placeholder="Buscar articulo..." aria-label="Buscar articulo" class="w-full rounded-lg border border-white/10 bg-white py-2 pl-8 pr-8 text-[10px] font-bold text-slate-900 placeholder-slate-400 shadow-sm outline-none transition-all focus:border-white focus:ring-2 focus:ring-white/20">
                                @if($search !== '')
                                    <a href="{{ route('client.servicios.index', ['per_page' => $perPage]) }}" class="absolute inset-y-0 right-0 flex items-center pr-3 text-slate-400 hover:text-rose-500 text-xs font-black">x</a>
                                @endif
                            </form>
                        </th>
