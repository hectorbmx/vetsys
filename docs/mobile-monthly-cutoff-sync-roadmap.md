...
| 2026-08-03 | Hotfix Cortes prod - bootstrap y endpoint | Completado parcial | El 500 de `GET /customers/statements` se debia a eager loading de `customer:id,full_name`; `full_name` es accessor y no columna. Se cambio a `customer:id,name,last_name` y `/mobile/bootstrap` ahora incluye `customer_statements`; la pantalla `Cortes` usa primero bootstrap y luego refresca el endpoint. Build Angular y `php -l` aprobados; falta verificar en prod tras deploy. |
| 2026-08-03 | Detalle cliente monthly - ultimos cortes | Completado parcial | En `/tabs/clientes/:id`, `monthly_cutoff` muestra al final hasta 3 cortes del cliente desde `GET /customers/statements?customer_id={id}&per_page=3`; `Ver mas` abre `/tabs/agenda?customer_id={id}` para ver solo los cortes de ese cliente. Build Angular aprobado; falta QA visual en dispositivo/prod. |
| 2026-08-03 | Cortes mobile - buscador y detalle | Completado parcial | La vista general `Cortes` agrega buscador por cliente y cada corte abre `/tabs/clientes/{customerId}/cortes/{statementId}`. El bloque de cortes del cliente tambien abre el detalle. Backend agrega `GET /customers/{customer}/statements/{statement}` con servicios incluidos y abonos del periodo, equivalente funcional al detalle web. `php -l` y build Angular aprobados; build reporta warning Sass por `@import` reutilizado. |
| 2026-08-03 | Abonos cliente monthly - filtro mes/anio | Completado parcial | Los abonos del resumen de `/tabs/clientes/:id` ahora son clicables y abren `/tabs/clientes/:id/pagos`; esa pantalla muestra historial de abonos con filtro por mes y anio, default mes actual y anio en curso. Backend `GET /payments` acepta `month` y `year`. `php -l` y build Angular aprobados; falta QA visual con cliente real. |
| 2026-08-03 | Registro abono mobile - fecha recibida | Completado parcial | El modal `Registrar abono` agrega `Fecha del abono` con default del dia actual y envia `paid_at` a `/payments`; backend valida `paid_at` y lo usa como `payments.created_at` para que filtros mensuales y cortes tomen la fecha real recibida. `php -l` y build Angular aprobados; falta QA registrando abono real. |
| 2026-08-03 | Registro abono web - fecha recibida | Completado parcial | El modal web de cliente (`client/customers/{id}`) agrega `Fecha del abono/pago` y `Client\PaymentController` valida `paid_at` para guardar `payments.created_at` con la fecha recibida. `php -l` aprobado; falta QA web registrando abono real. |
| 2026-08-03 | Abonos cliente monthly - lista completa por anio | Completado parcial | `/tabs/clientes/:id/pagos` carga todos los abonos del anio seleccionado por defecto y deja el mes como filtro opcional `Todos`; `GET /payments` ordena por `created_at` descendente para respetar la fecha capturada. El resumen de abonos en cliente muestra fecha en lugar de metodo de pago. Falta QA visual con pago de julio en tenant real. |
